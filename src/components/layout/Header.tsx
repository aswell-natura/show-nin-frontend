...
export default function Header({ onMenuToggle }: HeaderProps) {
  ...
  const rightSecondaryItems = [
    { label: "利用状況", icon: BarChart, onClick: () => navigate("/usage-status") },
    { label: "詳細設定", icon: Settings, onClick: () => navigate("/settings") },
    { label: "使い方", icon: HelpCircle },
    { label: "パスワード変更", icon: Lock, onClick: () => navigate("/password-change") },
  ];
  ...
  const billingMenuItems = [
    ...
    { label: "AIパケット追加", icon: CircleDollarSign, onClick: () => navigate("/ai-packets") },
    { label: "決済履歴", icon: History, onClick: () => navigate("/payment-history") },
    { label: "AIパケット履歴", icon: History, onClick: () => navigate("/ai-packet-history") },
  ];
