...
function TabsTrigger({ ref, value, children, className, itemClassName, ...props }: TabsTriggerProps) {
  ...
  return (
    <MotionHighlightItem value={value} className={cn('h-full', itemClassName)}>
      <motion.button
        ref={localRef}
...
function TabsContents({
  ...
  const activeIndex = Math.max(0, childrenArray.findIndex(
  ...
  return (
    <div data-slot='tabs-contents' className={cn('overflow-hidden', className)} {...props}>
      <motion.div className='-mx-2 flex h-full' animate={{ x: activeIndex * -100 + '%' }} transition={transition}>
        {childrenArray.map((child, index) => (
          <div key={index} className='h-full w-full shrink-0 px-2'>
            {child}
          </div>
        ))}
      </motion.div>
