...
export function GlobalDialogProvider({ children }: { children: ReactNode }) {
  const [dialogs, setDialogs] = useState<GlobalDialogOptions[]>([])

  const closeDialog = useCallback(() => {
    setDialogs((current) => {
      if (current.length === 0) return current
      const top = current[current.length - 1]
      top.onOpenChange?.(false)
      return current.slice(0, -1)
    })
  }, [])

  const openDialog = useCallback((options: GlobalDialogOptions) => {
    setDialogs((current) => [...current, options])
    options.onOpenChange?.(true)
  }, [])

  const value = useMemo(
    () => ({ openDialog, closeDialog, isOpen: dialogs.length > 0 }),
    [openDialog, closeDialog, dialogs.length],
  )
  ...
  return (
    <GlobalDialogContext.Provider value={value}>
      ...
      {dialogs.map((dialog, index) => {
        ...
        const isUnderneath = index < dialogs.length - 1
        ...
        return (
          ...
          >
            <DialogContent
              className={cn(
                "max-h-[calc(100vh-2rem)] grid-rows-[auto_minmax(0,1fr)_auto] gap-0 overflow-hidden rounded-2xl p-0 sm:w-[calc(100%-4rem)] transition-all duration-300 ease-in-out",
                sizeClassName[dialog.size ?? "lg"],
                isUnderneath && "opacity-45 pointer-events-none blur-[0.5px] shadow-sm",
              )}
              style={
                isUnderneath
                  ? {
