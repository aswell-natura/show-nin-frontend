      ...
      <div className="h-full flex flex-col bg-gray-50">
        ...
        <div className="bg-white border-b border-gray-200 px-4 md:px-6 py-4 shrink-0">
          <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
            <div>
              <h1 className="text-lg font-bold text-gray-900">議事録一覧</h1>
              <p className="mt-0.5 text-xs text-gray-400">録音から生成された音声議事録をまとめて管理します</p>
            </div>
          ...
          <div className="mt-4 flex flex-col sm:flex-row gap-2">
            <div className="relative">
              ...
              <input
                ...
                className="h-9 w-full sm:w-96 rounded-lg border border-gray-200 bg-white pl-8 pr-3 text-sm outline-none placeholder-gray-400 focus:ring-2 focus:ring-blue-100"
              />
            </div>
            <select
              value={linkFilter}
            ...
            <div className="overflow-x-auto">
              <table className="w-full min-w-[980px] table-fixed text-left">
                <thead className="bg-gray-50">
                  <tr>
                    ...
                    <th className="w-24 px-4 py-3 whitespace-nowrap text-xs font-medium text-gray-500">音声</th>
                    <th className="w-24 px-4 py-3 whitespace-nowrap text-xs font-medium text-gray-500 text-right">操作</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-100 text-sm">
                  {filteredMinutes.map(m => {
                    ...
                    return (
                      <tr key={m.id} className={`hover:bg-gray-50 ${isUnlinked ? 'bg-red-50/35' : ''}`}>
                        ...
                        <td className="px-4 py-3 text-gray-600 whitespace-nowrap">{formatDate(m.recording_date)}</td>
                        <td className="px-4 py-3 text-gray-600 whitespace-nowrap font-mono text-xs">{m.start_time}</td>
                        <td className="px-4 py-3 text-gray-600 whitespace-nowrap font-mono text-xs">{m.end_time}</td>
                        <td className="px-4 py-3">
                          {m.audio_url ? (
                        ...
                        <td className="px-4 py-3 text-right">
                          <button
                            ...
                            className="inline-flex items-center gap-1 rounded-lg border border-gray-200 bg-white px-3 py-1.5 text-xs font-medium text-gray-700 hover:bg-blue-50 hover:border-blue-200 hover:text-blue-700 transition-colors"
                          >
              ...
              {filteredMinutes.length === 0 && (
                <div className="py-14 text-center">
                  <p className="text-sm text-gray-400">条件に一致する議事録がありません</p>
                </div>
              )}
            </div>
          </div>
